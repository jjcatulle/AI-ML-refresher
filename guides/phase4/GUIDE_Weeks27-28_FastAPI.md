...
## Beginner Start Here
This week turns your model into a service others can use.

### What is an API?
An API is an interface where clients send requests and get responses over HTTP.

### Terms you must know first
- `Endpoint`: URL route like `/predict`.
- `Request`: input JSON sent by client.
- `Response`: output JSON returned by server.
- `Validation`: check input fields and types.
- `Health check`: endpoint that tells if service is alive.
- `Shadow mode`: run candidate model in background for comparison.
...
1. Build minimal `/health` and `/predict` endpoints.
2. Add validation and error handling.
3. Add guardrails and logging.
4. Add shadow comparison for safe upgrades.
