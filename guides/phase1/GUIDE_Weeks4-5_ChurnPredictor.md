...
## Beginner Start Here
No prior ML project experience is required.

### What this project does
You predict whether a customer is likely to leave (`churn=1`) or stay (`churn=0`).

### Terms you must know first
- `Classification`: predict category labels.
- `Target/Label`: output we want to predict (`churn`).
- `Train set`: data used to learn patterns.
...
### Modules used
- `pandas`: prepare data table.
- `sklearn`: train and evaluate models.
- `matplotlib`/`seaborn`: visualize distributions and results.

### How to study this guide
1. Understand target and business goal first.
2. Build a baseline model before tuning anything.
3. Improve one variable at a time (features, model, threshold).
4. Record each improvement with a metric.
