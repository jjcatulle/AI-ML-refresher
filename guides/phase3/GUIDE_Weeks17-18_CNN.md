...
## Beginner Start Here
If deep learning is new, start with this section before model code.

### What is a CNN?
A CNN is a neural network for images that learns patterns like edges, shapes, and textures.

### Terms you must know first
- `Tensor`: multi-dimensional array used in PyTorch/TensorFlow.
- `Epoch`: one full pass over training data.
- `Batch`: small subset processed at one time.
- `Loss`: how wrong the model is.
- `Backpropagation`: how gradients are computed to update weights.
- `Attention`: mechanism used by transformers to focus on important context.

### Modules used
- `torch`: deep learning framework.
- `torchvision`: datasets and transforms for images.
- `matplotlib`: visualize predictions and training curves.

### How to study this guide
1. Train a tiny baseline first.
2. Check tensor shapes in each step.
3. Compare baseline and improved model.
4. Document why metric changed.
